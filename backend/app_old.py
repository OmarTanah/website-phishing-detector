# app.py – Full ML version with class‑conditional imputation
...
def load_artifacts():
    ...
    if not all(os.path.exists(p) for p in (MODEL_PATH, X_TRAIN_PATH, Y_TRAIN_PATH)):
        app.logger.warning("Model or training data missing – heuristics will be used.")
        return
    try:
        _model = pickle.load(open(MODEL_PATH, "rb"))
        _X_train = pickle.load(open(X_TRAIN_PATH, "rb"))
        _y_train = pickle.load(open(Y_TRAIN_PATH, "rb"))
        ...
        app.logger.info("Model and training data loaded successfully.")
    except Exception as e:
        app.logger.error(f"Failed to load artifacts: {e}")

# ------------------------------------------------------------
# 2. Full feature extraction (with network calls)
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# 3. Class‑conditional multiple imputation prediction
# ------------------------------------------------------------
